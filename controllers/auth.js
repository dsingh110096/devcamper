...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, data: user, message: 'Email Sent Successfully' });
  } catch (err) {
    console.error(err);

    //Erasing entry from db if problem in sending email
    user.resetPasswordToken = undefined;
    user.resetPasswordExpire = undefined;

    await user.save({ validateBeforeSave: false });
    return next(new ErrorResponse('Email cound not be send', 500));
  }
});
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorResponse('Invalid token, reset password time expired...', 400)
    );
  }
